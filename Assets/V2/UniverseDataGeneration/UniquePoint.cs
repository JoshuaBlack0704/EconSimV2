...
namespace EconSimV2.Assets.V2.UniverseDataGeneration
{
    ...
    public class UniquePoint : IKDItem<UniquePoint>, IOctTreeItem<UniquePoint>
    {
        ...
        public int CompareToWithIndex(UniquePoint whatToCompareTo, int dimensionIndex)
        {
            ...
            //This Methode returns -1 for less than, 0 for equal to, 1 for greater than, and 2 for non-unique
            if ( dimensions[dimensionIndex] < whatToCompareTo.dimensions[dimensionIndex] )
            {
                return -1;
            }
            else if ( dimensions[dimensionIndex] == whatToCompareTo.dimensions[dimensionIndex] )
            {
                if ( Position == whatToCompareTo.Position )
                {
                    return 2;
                }
                return 0;
