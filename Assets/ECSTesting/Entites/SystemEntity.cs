...
namespace ECSTesting.Entites
{
    ...
    public static class SystemEntity
    {
        ...
        public static void EnterSystem(int id)
        {
            ...
            Entity[] shipCopys = shipArray.Where(o => em.GetComponentData<CurrentSystemID>(o).id == id).ToArray();
            NativeArray<Entity> shipCopyArray = new NativeArray<Entity>(shipCopys.Length, Allocator.Temp);
            shipCopyArray.CopyFrom(shipCopys);
            shipCopys = null;

            Entity[] planetsCopys = planetsArray.Where(o => em.GetComponentData<CurrentSystemID>(o).id == id).ToArray();
            ...
            Entity[] asteroidCopys = asteroidArray.Where(o => em.GetComponentData<CurrentSystemID>(o).id == id).ToArray();
