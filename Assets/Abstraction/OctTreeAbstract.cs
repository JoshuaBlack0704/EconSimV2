...
public class OctTreeAbstract<T> where T : IPositionController, IOctTreeItemAbstract<T>
{
    int maxCubeId = 0;
    ...
    public void ConnectSystems()
    {
        ...
        foreach (var point in SelfCollection.Values)
        {
            ...
            //    T connectedPoint = SelfCollection[finalSortedList[i].Id];
        ...
        //    {
        //        if (SelfCollection[connection.Id].Connections.Contains(item) != true)
        //        {
        //            Debug.LogError("here");
        //        }
        //    }
...
public interface IOctTreeItemAbstract<T>
{
    int Id { get; }
    List<int> OctTreeAddress { get; set; }
}

public struct distStructAbstract
{
    public int Id;
...
/// </summary>
[BurstCompile]
public struct DistCalcBatchAbstract : IJobParallelFor
{
    [ReadOnly]
