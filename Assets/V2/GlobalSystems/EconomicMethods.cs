...
namespace EconSimV2.Assets.V2.GlobalSystems
{
    public static class EconomicMethods
    {
        ...
        public static void RemoveOrReserve<T>(Entity entity, float withdraw = 0, float reserve = 0) where T : struct, IComponentData, IContainsResource
        {
            ...
            em.SetComponentData(entity, new T() { volume = currentVolume - withdraw, reservedVolume = currentReservation + reserve });
        }
        ...
        {
            MainSchedual.EventTicketHeapItem ticket = MainSchedual.AddToSystemV2(Random.Range(10, 100f), 5);
        ...
        public static void ReplaceAsteroid(int systemID)
        {
            ...
            em.SetComponentData(asteroid, new masterSystemId() { id = systemID });
            em.SetComponentData(asteroid, new Translation() { Value = rand.NextFloat3(0, system.size) });
            em.SetComponentData(asteroid, new asteroidId() { id = AsteroidMethods.MaxId });
            em.SetComponentData(asteroid, new FoodResource() { volume = 1000, reservedVolume = 0 });
            ...
            if ( UniverseGenerator.universe.inSystem && UniverseGenerator.universe.selectedSystem == systemID )
            {
                ...
                em.AddComponentData(newItem, new asteroidId() { id = em.GetComponentData<asteroidId>(asteroid).id });
                ...
                em.SetComponentData(newItem, new Rotation() { Value = rand.NextQuaternionRotation() });
            }
        }
    }
}