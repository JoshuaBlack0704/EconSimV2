...
namespace ECSTesting.DataOps
{
    ...
    public static class EntityPathFinder
    {
        ...
        internal class Node : IHeapItem<Node>
        {
            ...
            public Node(float3 _pos, SysComps.ConnectionData[] _connections, int _id, Entity definition) { position = _pos; connections = _connections; Id = _id; system = definition; }
        ...
        public static void GetEntityPath(int start, int end, NativeArray<Entity> knownPoints, NativeArray<Entity> explorablePoints, bool exploreable, out NativeArray<Entity> entityPath)
        {
            NativeList<int> usedNodes = new NativeList<int>(Allocator.Temp);
            ...
            while ( true )
            {
                ...
                {
                    int connection = currentNode.connections[c].target;
                    Entity connectionEntity = currentNode.connections[c].targetEntity;

                    bool inKnownSystems = knownPoints.Contains(connectionEntity);
                    bool inUnknownSystems = explorablePoints.Contains(connectionEntity);
            ...
            entityPath = new NativeArray<Entity>(path.Length, Allocator.Temp);
            entityPath.CopyFrom(path);
            for ( int i = 0; i < usedNodes.Length; i++ )
            {
                nodeCache[usedNodes[i]].Reset();
            }
        ...
        public static void Initialize()
        {
            ...
            foreach ( Entity ent in points )
            {
                ...
                nodeCache[id] = new Node(pos, connecitons, id, ent);
            }
